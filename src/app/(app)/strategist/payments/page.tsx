...
import type { FullStrategistMock, PaymentLink } from '@/lib/mocks/strategist-full';
...
function formatDueDate(date: Date | null): { main: string; sub: string } {
  if (!date) {
    return { main: '-', sub: '' };
  }
...
export default function StrategistPaymentsPage() {
  ...
  const strategistProfile = user ? getFullUserProfile(user) as FullStrategistMock | null : null;
  ...
  const payments = strategistProfile.paymentLinks;
  ...
  const overduePayments = payments.filter(p => {
    ...
    return (p.status === 'sent' || p.status === 'draft') && dueDate < now;
  });
            ...
            <table className="w-full">
                ...
                <tbody className="divide-y divide-zinc-100">
                  {sortedPayments.map(payment => {
                    const dueDate = formatDueDate(payment.dueDate);
                        ...
                        <td className="px-4 py-3">
                          <Badge 
                            variant={
                              ...
                              payment.status === 'cancelled' || payment.status === 'expired' ? 'destructive' : 
                              payment.status === 'sent' ? 'info' : 
                              'default'
                            }
                          >
                            {payment.status === 'paid' ? 'Paid' : 
                             payment.status === 'sent' ? 'Pending' : 
