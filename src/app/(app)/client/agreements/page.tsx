...
export default function ClientAgreementsPage() {
  ...
  const contractDocs = currentClient.documents.filter(d => d.category === 'contract');
        ...
        {contractDocs.length > 0 && (
          <div>
            {groupDocumentsByDate(
              [...contractDocs].sort((a, b) => b.createdAt.getTime() - a.createdAt.getTime())
            ).map(group => (
                ...
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
                  {group.documents.map(doc => (
                    <AgreementCard
                      ...
                      isSelected={selectedAgreements.has(doc.id)}
                      onToggleSelection={() => toggleAgreementSelection(doc.id)}
                    />
                  ))}
                </div>
              </div>
