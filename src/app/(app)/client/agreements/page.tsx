  ...
  return (
    <div className="pb-24">
      <div className="mx-auto flex w-full max-w-[642px] flex-col py-6">
        <h2 className="mb-6 text-2xl font-medium text-soft-white">Agreements</h2>
        ...
        {!isLoading && agreements.length === 0 && (
          <div className="flex flex-col items-center justify-center pt-24 pb-12 text-center">
            ...
            <p className="text-sm text-steel-gray">Agreements will appear here when your strategist sends them</p>
          </div>
        )}
        ...
        {!isLoading && pendingAgreements.length > 0 && (
          <div className="mb-8">
            <div className="mb-4 flex items-center gap-2">
              ...
              <h3 className="text-sm font-semibold text-soft-white">Action Required</h3>
        ...
        {!isLoading && signedAgreements.length > 0 && (
          <div className="mb-8">
            <div className="mb-4 flex items-center gap-2">
              ...
              <h3 className="text-sm font-semibold text-soft-white">Signed</h3>
            </div>
            <div className="flex flex-col gap-4">
        ...
        {!isLoading && otherAgreements.length > 0 && (
          <div className="mb-8">
            <div className="mb-4 flex items-center gap-2">
              ...
              <h3 className="text-sm font-semibold text-soft-white">Other</h3>
            </div>
            <div className="flex flex-col gap-4">
...
function AgreementItem({ agreement }: { agreement: ClientAgreement }) {
  ...
  return (
    <div className="flex items-center justify-between rounded-xl border border-white/10 bg-deep-navy p-4 transition-all hover:border-white/20">
      <div className="flex items-start gap-4">
        <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-white/8">
          <FileIcon className="h-5 w-5 text-steel-gray" weight="fill" />
        </div>
        <div className="flex flex-col">
          <h4 className="font-medium text-soft-white">{agreement.title}</h4>
          {agreement.description && (
            <p className="mt-1 line-clamp-2 text-sm text-steel-gray">{agreement.description}</p>
          )}
          {agreement.price && (
            <p className="mt-1 text-sm font-medium text-soft-white">
        ...
        <button
          ...
          className={`flex items-center gap-2 rounded-lg px-4 py-2 text-sm font-medium transition-colors ${
            hasCeremonyUrl
              ...
              : 'border border-white/10 bg-white/5 text-steel-gray cursor-not-allowed'
          }`}
      ...
      {isAgreementSigned(agreement.status) && (
        <button className="flex items-center gap-2 rounded-lg border border-white/10 px-4 py-2 text-sm font-medium text-steel-gray transition-colors hover:bg-white/8">
