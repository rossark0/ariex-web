...
import { fullClientMocks, FullClientMock } from '@/lib/mocks/client-full';
import { getAllStrategists, FullStrategistMock } from '@/lib/mocks/strategist-full';
import { getClientStatus } from '@/lib/client-status';
import { Envelope } from '@phosphor-icons/react/dist/ssr';
import { useRouter } from 'next/navigation';
import { useRoleRedirect } from '@/hooks/use-role-redirect';

// Get all strategists and clients for compliance oversight
const allStrategists = getAllStrategists();
const allClients = fullClientMocks;

// ============================================================================
// COMPLIANCE ACTIVITY TYPES & GENERATION
// ============================================================================
...
function generateComplianceActivities(
  strategists: FullStrategistMock[],
  clients: FullClientMock[]
): ComplianceActivity[] {
  const activities: ComplianceActivity[] = [];

  clients.forEach(client => {
    const clientName = client.user.name?.split(' ')[0] || 'Client';
    const strategist = strategists.find(s => s.user.id === client.strategistId);
    const strategistName = strategist?.user.name?.split(' ')[0] || 'Strategist';

    // Account created
    activities.push({
      id: `${client.user.id}-created`,
      strategistId: client.strategistId,
      strategistName,
      clientId: client.user.id,
      clientName,
      type: 'client_onboarded',
      description: `${strategistName} onboarded ${clientName}`,
      actionLabel: 'View',
...
function groupActivitiesByDate(activities: ComplianceActivity[]) {
  ...
  const groups: { label: string; activities: ComplianceActivity[] }[] = [];
...
const allActivities = generateComplianceActivities(allStrategists, allClients);
const groupedActivities = groupActivitiesByDate(allActivities);

// ============================================================================
// PENDING REVIEWS (Compliance-specific deadlines)
// ============================================================================
...
const pendingReviews: PendingReview[] = [
  { id: '1', title: 'Review Strategy - Sarah Johnson', date: inHours(3), type: 'review' },
  { id: '2', title: 'Monthly Compliance Audit Due', date: inDays(1), type: 'audit' },
  { id: '3', title: 'Review Document Package - Michael Chen', date: inDays(2), type: 'review' },
  { id: '4', title: 'Quarterly Report Submission', date: inDays(6), type: 'deadline' },
  { id: '5', title: 'Annual Compliance Review', date: inDays(12), type: 'audit' },
];
...
function EventRow({ event }: { event: PendingReview }) {
...
function ActivityRow({ activity }: { activity: ComplianceActivity }) {
  const router = useRouter();

  // Get the client to show their initials
  const client = allClients.find(c => c.user.id === activity.clientId);
  ...
  return (
    <div className="flex items-center gap-4 py-3">
      ...
      {/* Action Button */}
      <button
        onClick={() => router.push(`/compliance/strategists/${activity.strategistId}`)}
  ...
  return (
    <div className="flex min-h-full flex-col">
      <div className="flex-1">
        {/* Top Section - Pending Reviews */}
        <div className="shrink-0 bg-zinc-50/90 pt-8 pb-6">
          <div className="mx-auto w-full max-w-[642px]">
            <h2 className="mb-4 text-xl font-medium tracking-tight">Pending Reviews</h2>
          ...
          <div className="mx-auto flex w-full max-w-[642px] flex-col py-6">
            <div className="mb-4 flex w-full items-center justify-between">
              {/* <div>
                ...
                <p className="text-sm text-zinc-500">{allActivities.length} activities from {allClients.length} clients</p>
              </div> */}
              {/* <Button
                variant="outline"
                size="sm"
            ...
            <div className="flex flex-col">
              ...
              {allActivities.length === 0 && (
                <div className="flex flex-col items-center justify-center py-12 text-center">
                  ...
                  <p className="text-sm text-zinc-400">Platform activities will appear here</p>
                </div>
              )}
            </div>
          </div>
